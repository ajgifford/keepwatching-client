...
describe('ShowDetails', () => {
  ...
  describe('season watch status', () => {
    ...
    it('dispatches updateSeasonWatchStatus when season watch button is clicked', async () => {
      ...
      const watchIcons = screen.getAllByTestId('watch-status-icon');
      const seasonWatchButton = watchIcons[1].closest('button');

      if (seasonWatchButton) {
        fireEvent.click(seasonWatchButton);
      }
      ...
      await waitFor(() => {
        expect(mockDispatch).toHaveBeenCalledWith(
          updateSeasonWatchStatus({
            ...
            seasonStatus: expect.any(String),
          })
        );
      });
    });
  ...
  describe('episode watch status', () => {
    ...
    it('dispatches updateEpisodeWatchStatus when episode watch button is clicked', async () => {
      ...
      await waitFor(() => {
        const watchIcons = screen.getAllByTestId('watch-status-icon');
        // Find an episode watch button (not the season ones)
        const episodeWatchButton = watchIcons[watchIcons.length - 1].closest('button');

        if (episodeWatchButton) {
          fireEvent.click(episodeWatchButton);
        }
      });
      ...
      await waitFor(() => {
        expect(mockDispatch).toHaveBeenCalledWith(
          updateEpisodeWatchStatus({
            ...
            episodeStatus: expect.any(String),
          })
        );
      });
    });
