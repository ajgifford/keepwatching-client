...
jest.mock('../../common/media/scrollableMediaRow', () => ({
  ScrollableMediaRow: ({ title, items, renderItem }: any) => (
...
describe('MovieDetails', () => {
  ...
  describe('back button navigation', () => {
    ...
    it('shows correct tooltip for back to movies', () => {
      renderMovieDetails();

      const backButton = screen.getByLabelText('back');

      expect(backButton.closest('[title]') || backButton.parentElement?.closest('[title]')).toBeTruthy();
    });
  });
  ...
  describe('image rendering', () => {
    ...
    it('renders backdrop image', () => {
      ...
      const backdropImage = container.querySelector('[alt="Test Movie"]') as HTMLImageElement;
  ...
  describe('date formatting', () => {
    it('formats recent release date as full date', () => {
      renderMovieDetails();

      expect(screen.getByText('2024-01-15')).toBeInTheDocument();
    });

    it('formats old release date as year only', () => {
      const { useAppSelector } = require('../../../app/hooks');
      const { selectMovie } = require('../../../app/slices/activeMovieSlice');

      useAppSelector.mockImplementation((selector: any) => {
        if (selector === selectMovie) return { ...mockMovie, releaseDate: '2000-01-15' };
        if (selector === require('../../../app/slices/activeMovieSlice').selectCastMembers)
          return mockCastMembers;
        if (selector === require('../../../app/slices/activeMovieSlice').selectRecommendedMovies)
          return mockRecommendedMovies;
